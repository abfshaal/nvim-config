...
local function should_use_floating_diff()
  if config and config.diff_opts and config.diff_opts.floating == false then
    return false
  end
  return true
end
...
local function build_diff_text(old_file_path, new_file_path, new_file_contents)
  local new_content = new_file_contents or ""
  local old_content = ""
  if old_file_path and vim.fn.filereadable(old_file_path) == 1 then
    old_content = read_file_contents(old_file_path) or ""
  end

  local header_old = (old_file_path and old_file_path ~= "" and old_file_path) or "[current file]"
  local header_new = (new_file_path and new_file_path ~= "" and new_file_path) or "[proposed]"
  ...
  if diff_output and diff_output ~= "" then
    return string.format("--- %s\n+++ %s\n%s", header_old, header_new, diff_output)
  end

  if new_content == "" and old_content == "" then
    return string.format("%s is empty; proposed change introduces no content", header_new)
  end

  if new_content == old_content then
    return string.format("No changes between %s and proposed update", header_old)
  end

  return "Unable to compute diff for the proposed change"
end

local function open_floating_diff_view(old_file_path, new_file_path, new_file_contents, tab_name)
  local diff_text = build_diff_text(old_file_path, new_file_path, new_file_contents)
  diff_float.open(diff_text, tab_name)
end
...
function M._setup_blocking_diff(params, resolution_callback)
  ...
  local setup_success, setup_error = pcall(function()
    ...
    local use_floating = should_use_floating_diff()
    ...
    if (not use_floating) and config and config.diff_opts and config.diff_opts.open_in_new_tab then
    ...
    if not use_floating and not target_window and not created_new_tab then
      error({
        code = -32000,
    ...
    if use_floating then
      open_floating_diff_view(params.old_file_path, params.new_file_path, params.new_file_contents, tab_name)
    end
  end) -- End of pcall
...
local function current_diff_tab_name()
  local current_buffer = vim.api.nvim_get_current_buf()
  local tab_name = vim.b[current_buffer] and vim.b[current_buffer].claudecode_diff_tab_name or nil
  if tab_name then
    return tab_name
  end

  local float_tab = diff_float.current_tab_name()
  if float_tab then
    return float_tab
  end

  -- Fallback: return the first pending diff tab name if available
  for active_tab in pairs(active_diffs) do
    return active_tab
  end
...
function M.accept_current_diff()
  ...
  local diff_data = active_diffs[tab_name]
  if not diff_data or not diff_data.new_buffer or not vim.api.nvim_buf_is_valid(diff_data.new_buffer) then
    vim.notify("Diff buffer missing for " .. tab_name, vim.log.levels.ERROR)
    return
  end

  M._resolve_diff_as_saved(tab_name, diff_data.new_buffer)
end
